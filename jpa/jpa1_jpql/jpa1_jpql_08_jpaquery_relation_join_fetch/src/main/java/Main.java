...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        JPAUtil.checkData("select PRODUCT_ID, MANUFACTURER_ID, DESCRIPTION, MARKUP from PRODUCT");
        ...
        String jpql = "SELECT m FROM Manufacturer m, IN (m.productCollection) p WHERE p.markup > 20.0";
        Query query = em.createQuery(jpql);
        List<Manufacturer> manufacturers = query.getResultList();
        displayQueryResult(jpql, manufacturers);

        // Do Join fetch
        jpql = "SELECT m FROM Manufacturer m JOIN FETCH m.productCollection, IN (m.productCollection) p WHERE p.markup > 20.0";
